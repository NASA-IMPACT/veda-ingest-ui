...
describe('AWSHandler', () => {
  ...
  it('should request a page from the AWS handler', async () => {
    ...
    const templateFilePath = readdirSync(cdkOutPath).find((file) => file.startsWith('VikeStack-') && file.endsWith('.template.json'));
    ...
    const requestHandlerFolder = extractRequestHandlerPath(JSON.parse(cloudfrontTemplateJson), '/RequestHandler/Resource');
