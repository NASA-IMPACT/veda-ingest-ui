...
export class VikeStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: VikeStackProps) {
    ...
    const certificate = props.customStackProps?.certificate && typeof props.customStackProps?.certificate !== 'string' ? props.customStackProps?.certificate : undefined;
    ...
    const siteDomainName = domainName ? `${(subDomain?.length ?? 0 > 0) ? `${subDomain}.` : ''}${domainName}` : undefined;
    ...
    new s3deploy.BucketDeployment(this, 'DeployStaticAssets', {
      ...
      distributionPaths: ['/*'],
      prune: true,
      cacheControl: [s3deploy.CacheControl.maxAge(cdk.Duration.days(365)), s3deploy.CacheControl.sMaxAge(cdk.Duration.days(365))],
    });
